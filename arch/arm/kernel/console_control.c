			     ...
			     unsigned long count, void *data)
{
	 ...
	 */
	if (set_console_value > 1) {
		ret = -EINVAL;
		goto error;
	}

	printk(KERN_INFO "turning %s all consoles%s\n",
			set_console_value ? "on" : "off",
			set_console_value ? "" : " (except ram console)");

	acquire_console_sem();
	for (con = console_drivers; con != NULL; con = con->next) {
		if (strcmp(con->name, "ram")) {
			console_value = set_console_value;
	...
	/*
	 * console suspend should be enabled when consoles other than
	 * ram console are active as we don't want to output data to
	 * devices being suspended
	 */
	console_suspend_enabled = set_console_value;

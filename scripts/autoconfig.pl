...
sub usage($)
{
    ...
    print "Usage: defconfig.pl -t <action>[:<key>[=<value]] <products>\n\n";
    ...
    # The defconfigs for the PDPs do not follow the naming convention
    # so they are added manually. This should be removed once we stop
    # supporting the PDPs.
    print "\tpdp\n";
    print "\tpdp_r2\n";
...
sub perform_task($$$$)
{
    ...
    system "make ARCH=arm oldconfig KCONFIG_NOTIMESTAMP=true > /dev/null";
    copy(".config", "$config_dir/$defconfig") or die "failed to copy file";
}
...
# supporting the PDPs.
if (@products == 0) {
    push @defconfigs, "pdp_defconfig";
    push @defconfigs, "pdp_r2_defconfig";
} else {
    if (grep {$_ eq "pdp"} @products) {
        push @defconfigs, "pdp_defconfig";
    }
    if (grep {$_ eq "pdp_r2"} @products) {
        push @defconfigs, "pdp_r2_defconfig";
    }
}

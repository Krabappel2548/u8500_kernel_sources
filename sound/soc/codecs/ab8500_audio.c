...
/* VAPE OPP registers in bank AB8500_REGU_CTRL2 (0x04) */
...
/* Saves the setting for VAPE 50% OPP and then replaces it with
 * the setting for VAPE 100% OPP.
 */
int ab8500_codec_override_vape_opp50(void)
{
	int ret = 0;
	...
	if (!burst_enabled) {
		...
		opp100_val = ab8500_codec_read_reg(ab8500_codec,
			AB8500_REGU_CTRL2, AB8500_VAPESEL1_REG);
		if (opp100_val < 0)
			goto out;

		opp50_val = ab8500_codec_read_reg(ab8500_codec,
			AB8500_REGU_CTRL2, AB8500_VAPESEL2_REG);
		if (opp50_val < 0)
			goto out;

		ret = ab8500_codec_write_reg(ab8500_codec,
			AB8500_REGU_CTRL2, AB8500_VAPESEL2_REG, opp100_val);
...
/* Restores the original APE OPP 50% value.
 */
int ab8500_codec_restore_vape_opp50(void)
{
	if (burst_enabled) {
		burst_enabled = 0;
		return ab8500_codec_write_reg(ab8500_codec,
			AB8500_REGU_CTRL2, AB8500_VAPESEL2_REG, opp50_val);
	} else {
		return 0;
	}
}
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	if (((sidconf & BMASK(REG_SIDFIRCONF_FIRSIDBUSY)) != 0)) {
		pr_err("%s: Sidetone FIR is busy. Reset and try again!\n",
			__func__);
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	(BMASK(REG_DAPATHENA_ENDA1) | BMASK(REG_DAPATHENA_ENDA2) |
	BMASK(REG_DAPATHENA_ENDA3) | BMASK(REG_DAPATHENA_ENDA4))) > 0;
...
{
	int ret;
	unsigned int set_mask_hs, clear_mask_hs;
	if (ucontrol->value.enumerated.item[0] != 0) {
		clear_mask_hs = 0;
		set_mask_hs = BMASK(REG_DAPATHENA_ENDA1) |
		BMASK(REG_DAPATHENA_ENDA2) | BMASK(REG_DAPATHENA_ENDA3) |
...
static const struct soc_enum enum_headset_left =
			SOC_ENUM_SINGLE(0, 0, 2, enum_dis_ena);
...
static const struct soc_enum enum_headset_right =
			SOC_ENUM_SINGLE(0, 0, 2, enum_dis_ena);
...
static const struct soc_enum enum_lineout_left =
			SOC_ENUM_SINGLE(0, 0, 2, enum_dis_ena);
...
static const struct soc_enum enum_lineout_right =
			SOC_ENUM_SINGLE(0, 0, 2, enum_dis_ena);
...
static const struct soc_enum enum_ihf_or_lineout_select =
			SOC_ENUM_SINGLE(0, 0, 2,
					enum_ihf_or_lineout_select_sel);
...
static const struct snd_soc_dapm_widget ab8500_dapm_widgets[] = {
	...
	SND_SOC_DAPM_MIXER("HSL DAC Mute", REG_MUTECONF,
			REG_MUTECONF_MUTDACHSL, INVERT, NULL, 0),
	SND_SOC_DAPM_MIXER("HSR DAC Mute", REG_MUTECONF,
			REG_MUTECONF_MUTDACHSR, INVERT, NULL, 0),
	...
	SND_SOC_DAPM_SWITCH("LineIn Left", SND_SOC_NOPM,
			0, 0, dapm_linl_mute),
	SND_SOC_DAPM_SWITCH("LineIn Right", SND_SOC_NOPM,
			0, 0, dapm_linr_mute),
...
static const struct snd_soc_dapm_route dapm_routes[] = {
	...
	{"Earpiece or LineOut Mono Source", "IHF Left",
...
static const char * const enum_earselcm[] = {
	"0.95V", "1.10V", "1.27V", "1.58V" };
...
static struct snd_kcontrol_new ab8500_snd_controls[] = {
	...
	SOC_ENUM_EXT("Digital Interface 0 FIFO Enable Switch", soc_enum_if0fifoen,
		if0_fifo_enable_control_get, if0_fifo_enable_control_put),
		...
		unsigned int fmt)
{
	...
	ab8500_codec_update_reg_audio(codec, REG_DIGIFCONF4,
		clear_mask, set_mask);
...
static int ab8500_codec_set_word_length_if1(struct snd_soc_codec *codec,
			unsigned int wl)
{
	unsigned int clr_mask, set_mask;

	clr_mask = BMASK(REG_DIGIFCONF4_IF1WL1) | BMASK(REG_DIGIFCONF4_IF1WL0);
	set_mask = 0;
			...
			unsigned int delay)
{
	...
	ab8500_codec_update_reg_audio(codec, REG_DIGIFCONF4, clear_mask,
		set_mask);
			...
			unsigned char speed_right_neg)
{
	...
	vibra_on = speed_left_pos | speed_left_neg |
			speed_right_pos | speed_right_neg;
	if (!vibra_on) {
		speed_left_pos = 0;
		speed_left_neg = 0;
		speed_right_pos = 0;
	...
	ab8500_codec_update_reg_audio(ab8500_codec, REG_PWMGENCONF1,
			REG_MASK_NONE, set_mask);

	if (speed_left_pos > REG_PWMGENCONFX_PWMVIBXDUTCYC_MAX)
		speed_left_pos = REG_PWMGENCONFX_PWMVIBXDUTCYC_MAX;
	ab8500_codec_update_reg_audio(ab8500_codec, REG_PWMGENCONF3,
			REG_MASK_ALL, speed_left_pos);

	if (speed_left_neg > REG_PWMGENCONFX_PWMVIBXDUTCYC_MAX)
		speed_left_neg = REG_PWMGENCONFX_PWMVIBXDUTCYC_MAX;
	ab8500_codec_update_reg_audio(ab8500_codec, REG_PWMGENCONF2,
			REG_MASK_ALL, speed_left_neg);

	if (speed_right_pos > REG_PWMGENCONFX_PWMVIBXDUTCYC_MAX)
		speed_right_pos = REG_PWMGENCONFX_PWMVIBXDUTCYC_MAX;
	ab8500_codec_update_reg_audio(ab8500_codec, REG_PWMGENCONF5,
			REG_MASK_ALL, speed_right_pos);

	if (speed_right_neg > REG_PWMGENCONFX_PWMVIBXDUTCYC_MAX)
		speed_right_neg = REG_PWMGENCONFX_PWMVIBXDUTCYC_MAX;
	ab8500_codec_update_reg_audio(ab8500_codec, REG_PWMGENCONF4,
			REG_MASK_ALL, speed_right_neg);

	if (vibra_on) {
		clear_mask = 0;
		set_mask = BMASK(REG_ANACONF4_ENVIB1) |
			BMASK(REG_ANACONF4_ENVIB2);
	} else {
		clear_mask = BMASK(REG_ANACONF4_ENVIB1) |
			BMASK(REG_ANACONF4_ENVIB2);
		set_mask = 0;
	};
	ab8500_codec_update_reg_audio(ab8500_codec, REG_ANACONF4,
		clear_mask, set_mask);
}
...
int ab8500_audio_set_word_length(struct snd_soc_dai *dai, unsigned int wl)
{
	...
	clear_mask = BMASK(REG_DIGIFCONF2_IF0WL0) |
		BMASK(REG_DIGIFCONF2_IF0WL1);
	set_mask = 0;
	...
	ab8500_codec_update_reg_audio(codec, REG_DIGIFCONF2,
		clear_mask, set_mask);
...
int ab8500_audio_anc_fir(unsigned int bank, unsigned int param)
{
	int ret;

	if (param == 0 && bank == 0) {
		ret = ab8500_codec_update_reg_audio_locked(
			ab8500_codec, REG_ANCCONF1,
			REG_MASK_NONE, BMASK(REG_ANCCONF1_ANCFIRUPDATE));
		if (ret < 0)
			return ret;
	}

	ret = ab8500_codec_write_reg_audio(ab8500_codec, REG_ANCCONF5,
		anc_fir_cache[param] >> 8 & REG_MASK_ALL);
	if (ret < 0)
		return ret;
	ret = ab8500_codec_write_reg_audio(ab8500_codec, REG_ANCCONF6,
...
int ab8500_audio_anc_iir(unsigned int bank, unsigned int param)
{
	int ret;
	...
	if (param == 0) {
		if (bank == 0) {
			...
			ret = ab8500_codec_update_reg_audio_locked(
				ab8500_codec, REG_ANCCONF1, REG_MASK_NONE,
				BMASK(REG_ANCCONF1_ANCIIRUPDATE));
			if (ret < 0)
				return ret;
		}
	} else if (param > 3) {
		ret = ab8500_codec_write_reg_audio(ab8500_codec, REG_ANCCONF7,
			REG_MASK_NONE);
		if (ret < 0)
			return ret;
		ret = ab8500_codec_write_reg_audio(ab8500_codec, REG_ANCCONF8,
	...
	if (configure_fir) {
		for (bank = 0; bank < AB8500_NR_OF_ANC_COEFF_BANKS; bank++) {
			for (param = 0; param < REG_ANC_FIR_COEFFS; param++) {
				ret = ab8500_audio_anc_fir(bank, param);
				if (ret < 0)
					goto cleanup;
			}
		}
	...
	if (configure_iir) {
		for (bank = 0; bank < AB8500_NR_OF_ANC_COEFF_BANKS; bank++) {
			for (param = 0; param < REG_ANC_IIR_COEFFS; param++) {
				ret = ab8500_audio_anc_iir(bank, param);
				if (ret < 0)
					goto cleanup;
			}
		}
...
int ab8500_audio_set_adcm(enum ab8500_audio_adcm req_adcm)
{
	...
	if (AB8500_AUDIO_ADCM_FORCE_UP == req_adcm ||
		...
		adcm_anaconf5 = ab8500_codec_read_reg_audio(ab8500_codec,
				REG_ANACONF5);
		adcm_muteconf = ab8500_codec_read_reg_audio(ab8500_codec,
				REG_MUTECONF);
		adcm_anaconf4 = ab8500_codec_read_reg_audio(ab8500_codec,
				REG_ANACONF4);
...
static int ab8500_codec_add_widgets(struct snd_soc_codec *codec)
{
	...
	ret = snd_soc_dapm_add_routes(codec, dapm_routes,
		...
		struct snd_soc_dai *dai)
{
	...
	ab8500_codec_update_reg_audio(codec, REG_DIGIFCONF3,
			clear_mask, set_mask);
}
		...
		unsigned int fmt)
{
	...
	ab8500_codec_update_reg_audio(codec, REG_DIGIFCONF1,
		clear_mask, set_mask);
		...
		int slots, int slot_width)
{
	...
	case 1:
		...
		ab8500_codec_update_reg_audio(codec, REG_DASLOTCONF1,
			clear_mask, 9);
		ab8500_codec_update_reg_audio(codec, REG_DASLOTCONF3,
			clear_mask, 9);
		break;
	case 2:
		...
		ab8500_codec_update_reg_audio(codec, REG_DASLOTCONF3,
			clear_mask, 9);
		ab8500_codec_update_reg_audio(codec, REG_DASLOTCONF2,
			clear_mask, 11);
		ab8500_codec_update_reg_audio(codec, REG_DASLOTCONF4,
			clear_mask, 11);
	...
	case 2:
		...
		break;
	case 8:
		pr_debug("%s: In 8-channel mode AD-to-slot mapping \
			is set manually.", __func__);
		break;
	default:

				...
				struct snd_soc_dai *dai)
{
	...
	dev_info(ab5500_dev, "ab5500_pcm_hw_params\n");
	if (!ab5500_dev) {
		pr_err("%s: The AB5500 codec driver not initialized.\n",
		       __func__);
		return -EAGAIN;
...
static int ab5500_set_dai_fmt(struct snd_soc_dai *codec_dai, unsigned int fmt)
{
	...
	if (codec_dai->playback_active && codec_dai->capture_active) {
		if ((read_reg(iface) & I2Sx_MODE_MASK) == val) {
			dev_info(ab5500_dev,
				"%s: DAI format set "
				"by an existing stream. as %u\n",
				__func__, read_reg(iface));
			return 0;
		}
		else {
			dev_err(ab5500_dev,
...
static inline void init_playback_gain(void)
{
	...
	mask_set_reg(RX1_DPGA, RXx_DPGA_MASK, 0x2C << RXx_DPGA_SHIFT);
	mask_set_reg(RX2_DPGA, RXx_DPGA_MASK, 0x2C << RXx_DPGA_SHIFT);

...
static char *regulator_names[] = {"v-ana"};
...
static int mmio_power_init(struct mmio_platform_data *pdata)
{
	...
	for (i=0; i<pincount; i++) {
		...
		dev_dbg(pdata->dev, ".. request GPIO[%2.2d]\n", PIN_NUM(poweron_pins[i]));
		err = gpio_request(PIN_NUM(poweron_pins[i]), "mmio power LDO");

		if (err) {
			dev_err(pdata->dev, "%s(): error=%d while requesting LDO GPIO[%2.2d]\n",
				__func__,
				err,
				PIN_NUM(poweron_pins[i]));
			...
			for (i--;i>=0;i--)
			{
				dev_dbg(pdata->dev, ".. freeing up GPIO[%2.2d]\n", PIN_NUM(poweron_pins[i]));
...
static void mmio_power_exit(struct mmio_platform_data *pdata)
{
	...
	for (i=(pincount-1); i>=0; i--) {
		...
		dev_dbg(pdata->dev, ".. free GPIO[%2.2ld]\n",
					PIN_NUM(poweron_pins[i]));
		gpio_free(PIN_NUM(poweron_pins[i]));
	}
				      ...
				      int is_active_high)
{
	...
	case MMIO_ENABLE_XSHUTDOWN_HOST:
		...
		dev_dbg(pdata->dev, "ENABLE_XSHUTDOWN_HOST: GPIO[%d] set to %s\n",
	...
	case MMIO_ENABLE_XSHUTDOWN_FW:
		...
		dev_dbg(pdata->dev, "ENABLE_XSHUTDOWN_FW: GPIO[%d] set to Alt-%d\n",
	...
	case MMIO_DISABLE_XSHUTDOWN:
		...
		dev_dbg(pdata->dev, "DISABLE_XSHUTDOWN: GPIO[%d] set to Alt-%d\n",
				...
				enum mmio_select_i2c_t select)
{
	...
	dev_dbg(pdata->dev, "Board %s() Enter, pdata->camera_slot\n", __func__, pdata->camera_slot);

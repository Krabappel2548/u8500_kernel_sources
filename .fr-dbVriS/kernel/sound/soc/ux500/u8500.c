...
static int gpio_reg=0;
static int gpio_enable=0;

static int external_amp_set_info(struct snd_kcontrol* kcontrol,
...
static int external_amp_set_get(struct snd_kcontrol* kcontrol,
				struct snd_ctl_elem_value* ucontrol){

	ucontrol->value.integer.value[0]=gpio_enable;
	return 0;
}

static int external_amp_set_put(struct snd_kcontrol* kcontrol,
				struct snd_ctl_elem_value* ucontrol){

	gpio_enable=ucontrol->value.integer.value[0];
	int ret=-1;

	if(1==gpio_enable){
		ret = gpio_request(gpio_reg, "External AMP");
		if (ret < 0) {
			pr_err("gpio request failed\n");
			goto cleanup;
		}
		ret = gpio_direction_output(gpio_reg, gpio_enable);
		if (ret < 0) {
			pr_err("gpio direction output failed\n");
			gpio_free(gpio_reg);
			goto cleanup;
		}
		gpio_set_value(gpio_reg, gpio_enable);
	}else if(0==gpio_enable){
		gpio_set_value(gpio_reg, gpio_enable);
		gpio_free(gpio_reg);
		ret=0;
	}
...
static struct snd_kcontrol_new external_amp_set_ctrl={
	.iface=SNDRV_CTL_ELEM_IFACE_MIXER,
	.name="External AMP Set",
	.index=0,
	.access=SNDRV_CTL_ELEM_ACCESS_READWRITE,
	.private_value=0xffff,
	.info=external_amp_set_info,
	.get=external_amp_set_get,
	.put=external_amp_set_put
};


static int external_amp_gpio_info(struct snd_kcontrol* kcontrol,
				 struct snd_ctl_elem_info* uinfo){
...
static int external_amp_gpio_get(struct snd_kcontrol* kcontrol,
				struct snd_ctl_elem_value* ucontrol){

	ucontrol->value.integer.value[0]=gpio_reg;
	return 0;
}

static int external_amp_gpio_put(struct snd_kcontrol* kcontrol,
				struct snd_ctl_elem_value* ucontrol){

	gpio_reg = ucontrol->value.integer.value[0];
	return 0;

}

static struct snd_kcontrol_new external_amp_gpio_ctrl={
	.iface=SNDRV_CTL_ELEM_IFACE_MIXER,
	.name="External AMP GPIO",
	.index=0,
	.access=SNDRV_CTL_ELEM_ACCESS_READWRITE,
	.private_value=0xffff,
	.info=external_amp_gpio_info,
	.get=external_amp_gpio_get,
	.put=external_amp_gpio_put
};
...
static int __init u8500_soc_init(void)
{
	...
	ret = snd_ctl_add(u8500_drvdata.snd_card, snd_ctl_new1(&external_amp_gpio_ctrl, NULL));

	if(ret){
	  pr_err("snd_ctl_new1(external_amp_gpio_ctrl), ret: %d\n", ret);
	}

	ret = snd_ctl_add(u8500_drvdata.snd_card, snd_ctl_new1(&external_amp_set_ctrl, NULL));

	if(ret){
	  pr_err("snd_ctl_new1(external_amp_set_ctrl), ret: %d\n", ret);
	}

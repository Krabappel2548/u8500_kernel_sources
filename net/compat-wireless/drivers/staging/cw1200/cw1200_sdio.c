...
static int cw1200_pm_notifier(struct notifier_block *notifier,
			      unsigned long pm_event,
			      void *unused)
{
	struct sbus_priv *self = container_of(notifier,
					      struct sbus_priv,
					      pm_notifier);
	struct cw1200_common *priv = self->core;

	switch (pm_event) {
	case PM_HIBERNATION_PREPARE:
	case PM_SUSPEND_PREPARE:
		if (atomic_read(&priv->bh_suspend) != CW1200_BH_SUSPENDED &&
		    priv->mode == NL80211_IFTYPE_UNSPECIFIED)
			cw1200_pm_clear_wen(priv);
		break;
...
static void cw1200_register_pm_notifier(struct sbus_priv *self)
{
	self->pm_notifier.notifier_call = cw1200_pm_notifier;
	register_pm_notifier(&self->pm_notifier);
}

static void cw1200_unregister_pm_notifier(struct sbus_priv *self)
{
	unregister_pm_notifier(&self->pm_notifier);
}
			      ...
			      const struct sdio_device_id *id)
{
	...
	cw1200_register_pm_notifier(self);
...
static void cw1200_sdio_disconnect(struct sdio_func *func)
{
	...
	if (self) {
		cw1200_unregister_pm_notifier(self);
		if (self->core) {

...
static void wake_up_chip(struct uart_info *uart_info)
{
	...
	if (CHIP_ASLEEP == uart_info->sleep_state ||
		...
		/* Add work so we will go to sleep */
...
static void uart_set_chip_power(struct cg2900_chip_dev *dev, bool chip_on)
{
	...
	if (chip_on) {
		...
		if (uart_info->rx_skb) {
			 ...
			 * correctly by driver.
			 */
			dev_dbg(MAIN_DEV, "Power off in the middle of data"
				 "receiving? Reseting state machine.\n");
...
static int __devinit cg2900_uart_probe(struct platform_device *pdev)
{
	...
	uart_info->pm_qos_latency = pm_qos_add_request(PM_QOS_CPU_DMA_LATENCY,
				PM_QOS_DEFAULT_VALUE);

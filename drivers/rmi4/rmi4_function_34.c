				  ...
				  const char *buf, size_t count)
{
	...
	if (strstr(buf, RMI4_ENTER_BL_MODE_CMD)) {
		...
		dev_info(&fdev->dev, "%s - Successfully entered BL mode\n",
			 __func__);
	} else if (strstr(buf, RMI4_ERASE_CONFIG_CMD)) {
		...
		if (err)
			...
			dev_info(&fdev->dev,
				 ...
				 __func__);
	} else if (strstr(buf, RMI4_ERASE_ALL_CMD)) {

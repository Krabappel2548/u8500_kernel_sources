...
unknown:
		...
		/* The condition is same as above, but all devices registered
		 * are not in struct usb_configuration's interface field, but
		 * in the functions field, such as when device support
		 * rebind_config.
		 */
		if (value < 0) {
			struct usb_configuration	*c;

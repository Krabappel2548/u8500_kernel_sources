		...
		const struct usb_ctrlrequest *pControlRequest)
{
	...
	/* Handle microsoft os string descriptor */
	if (pControlRequest->bRequestType ==
		(USB_DIR_IN | USB_TYPE_STANDARD | USB_RECIP_DEVICE)
		&& uRequest == USB_REQ_GET_DESCRIPTOR
		&& (uValue >> 8) == USB_DT_STRING
		&& (uValue & 0xFF) == MSOS_STRING_DESC_INDEX) {

		uLength = min(uLength,
			(u16) sizeof(str_ms_os_desc));
		memcpy(pUsbRequest->buf, &str_ms_os_desc, uLength);
		return uLength;
	}

	/* Handle microsoft os feature descriptor */
	if (pControlRequest->bRequestType == MSOS_FEATURE_DESCRIPTOR
		&& uRequest == MSOS_VENDOR_CODE
		&& uIndex == MSOS_VENDOR_EXTN_INDEX) {
		int i;
		int func_num = 0;
		struct ms_ext_compatID_desc_head *head;
		struct ms_ext_compatID_desc_data *data;
		struct usb_function *func;
		struct usb_configuration *mtp_config = f->config;

		head = (struct ms_ext_compatID_desc_head *)pUsbRequest->buf;
		data = (struct ms_ext_compatID_desc_data *)(head + 1);

		/* zero clear */
		memset(pUsbRequest->buf, 0x00, cdev->bufsiz);

		/* data section */
		for (i = 0; i < MAX_CONFIG_INTERFACES; i++) {
			func = mtp_config->interface[i];
			...
			data->reserved1 = 1;
			if (!strcmp(func->name, gszDrvShortName))
				memcpy(data->compatibleID, "MTP", 3);
			data++;
			func_num++;
		}
		nResult = sizeof(*head) + (sizeof(*data) * func_num);

		/* header section */
		head->dwLength = nResult;
		head->bcdVersion = MSOS_DESC_VERSION;
		head->wIndex = MSOS_FEATURE_INDEX;
		head->bCount = func_num;
	}

	if (uRequestType == USB_TYPE_CLASS) {
		switch (uRequest) {
		...
		case MTP_CONTROL_CANCEL_TRANSACTION: /* cancel request */
		{
			...
			/* device busy */
			pDevice->m_nDeviceStatus = enStatusDeviceBusy;
			...
			while ((pUsbReq = PullUsbRequest(pDevice,
							&pDevice->m_RxDone))) {
				MTPDBG(pDevice,
					"submitting buffer to idle queue\n");
				PushUsbRequest(pDevice,
						&pDevice->m_RxIdle, pUsbReq);
			}
			usb_ep_fifo_flush(pDevice->m_pEpOut);
			/*usb_ep_fifo_flush(pDevice->m_pEpIn);
			 usb_ep_fifo_flush(pDevice->m_pEpIntr);*/
			cancelTrans = 1;
			kill_fasync(&pDevice->m_AsyncQueue,
					MTP_INTR_CODE, POLL_IN);
			wake_up(&pDevice->m_Readwq);
			...
			nResult = uLength;
			MTPDBG(pDevice,
				"After kill_fasync of cancel transaction\n");
			break;
		}
		...
		default:
			MTPERROR(pDevice,
				"Unsupported SETUP type: %02x, uRequest: %02x,"
				"uIndex: %04x, uValue: %04x, uLength: %04x\n",
				pControlRequest->bRequestType,
				uRequest, uIndex, uValue, uLength);
		}
	}
...
static void MtpGadgetDisconnect(struct usb_function *f)
{
	...
	pDevice = func_to_dev(f);
	/* clear all rx done buffer and put to idle buffer */
	while ((pUsbReq = PullUsbRequest(pDevice, &pDevice->m_RxDone))) {
		MTPDBG(pDevice, "submitting buffer to idle queue\n");
		PushUsbRequest(pDevice, &pDevice->m_RxIdle, pUsbReq);
	}

	if (pDevice->m_nMtpEvent != enUsbDisconnected)
		usb_ep_fifo_flush(pDevice->m_pEpOut);
...
requeue_req:
		...
		if (pDevice->m_uReadCount > 0) {
			...
			/* During cancellation of a transaction we flush
			 * the fifo and the fifo should be cleared. Instead
			 * there are some data left which are already handled.
			 * This workaround ignore those data which are already
			 * handled and only read the "correct" data.*/
			if ((nCount == 12) &&
				cancelTrans &&
				(pDevice->m_uReadCount
					> USB_EP_BULK_MAX_SIZE)) {
				tempCnt = pDevice->m_uReadCount
						% USB_EP_BULK_MAX_SIZE;
				pDevice->m_pReadBuff +=
					(pDevice->m_uReadCount - tempCnt);
				pDevice->m_uReadCount = tempCnt;
			}
			cancelTrans = 0;
...
static int mtp_rebind_config(struct usb_configuration *c,
				struct usb_function *f)
{
	int id;
...
static struct android_usb_function mtp_function = {
	...
	.rebind_config = mtp_rebind_config,
};

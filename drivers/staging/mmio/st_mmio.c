 ...
 * There are two ioctls that may use this:
 * MMIO_CAM_LOAD_XP70_FW and MMIO_CAM_ISP_WRITE
 */
static u16 copybuff[512];
static DEFINE_MUTEX(copybuff_mutex);
					...
					void __iomem *src_buf, u32 size)
{
	...
	err = mutex_lock_interruptible(&copybuff_mutex);
	if (err) {
		dev_err(info->dev, "interrupted mutex lock: %d\n", err);
		return err;
	}
			     ...
			     struct xp70_fw_t *xp70_fw)
{
	...
	return 0;
err_exit:
	dev_err(info->dev, "Loading XP70 fw failed: %d\n", err);
	return -EFAULT;
}
		...
		unsigned long enable)
{
	...
	dev_dbg(info->dev, "%s enable=%d\n", __func__, info->xshutdown_enabled);
...
static int mmio_cam_initboard(struct mmio_info *info)
{
	...
	dev_dbg(info->dev, "%s: IP firmware controls XRST for main cam!\n",
				__func__);
	...
	/*
	 * Note to SEMC: can not do MMIO_ACTIVATE_I2C_HOST before power
	 * is turned ON due to pull-up resistors of IPI2C SDA & SCL
	 */
	if (err) {
			  ...
			  struct isp_write_t *isp_write_p)
{
	...
	/* skip malloc: do the copy in chunks using 'copybuff' */
	for (offset = 0; offset < size; ) {
		...
		if (copy_from_user(data,
			((u8 *)isp_write_p->data) + offset, count)) {
		...
		for (i = 0; i < count; ) {
			addr = (void *)(info->siabase +
				t1_to_arm(isp_write_p->t1_dest + offset,
				info->siabase, &mem_page));
...
ssize_t mmio_read(struct file *filp,
		  char __user *buf,
		  size_t count,
		  loff_t *f_pos)
{
	if (!buf)
		return -EINVAL;
	if (!info->nvm_data_ptr)
		return -EPERM;
	/* Check if user is not requesting data more than what is available */
	if (count > info->nvm_data_size)
		return -EINVAL;

	/* copy count from kernel buffer to user space buffer */
	/* copy_to_user returns 0 on success */
	if (copy_to_user(buf, info->nvm_data_ptr, count))
		return -EFAULT;

	return count;
}

ssize_t mmio_write(struct file *filp,
		   ...
		   loff_t *f_pos)
{
	...
	/* If nvm data is present, delete previous data
	 * and allocate memory for new one.
	 * Also, reset the size of NVM data stored internally
	 */
	if (info->nvm_data_size != 0) {
		kfree(info->nvm_data_ptr);
		info->nvm_data_size = 0;
	}
	/* allocate memory for NVM data */
	info->nvm_data_ptr = kmalloc(count, GFP_KERNEL);
	if (!info->nvm_data_ptr)
		return -ENOMEM;

	/* copy count from a user space buffer to the kernel buffer */
	/* copy_from_user returns 0 on success */
	if (copy_from_user(info->nvm_data_ptr, buf, count))
		return -EFAULT;
	/* Update internally, size of NVM data stored */
	info->nvm_data_size = count;
	return count;
}
...
static ssize_t attr_nvm_data_size_show(struct device *dev,
					struct device_attribute *attr,
					char *buf)
{
...
static struct device_attribute xp70_device_attrs[] = {
	...
	__ATTR(nvm_data_size, S_IRUGO, attr_nvm_data_size_show, NULL),
	__ATTR_NULL
};

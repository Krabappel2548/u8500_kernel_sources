...
struct cyttsp {
	...
	struct input_dev *input_airtouch;
	struct input_dev *input_last_used;
						...
						struct input_dev *input_device)
{
	...
	for (id = 0; id < CY_NUM_TRK_ID; id++) {
		if (ts->act_trk[id] == CY_NTCH ||
				(input_device == ts->input_last_used &&
				t->cur_trk[id] != CY_NTCH))
			continue;

		input_report_abs(ts->input_last_used, ABS_MT_TRACKING_ID,
					id - 1);
		input_report_abs(ts->input_last_used, ABS_MT_TOUCH_MAJOR,
					CY_NTCH);
		input_report_abs(ts->input_last_used, ABS_MT_WIDTH_MAJOR,
					t->tool_width);
		input_report_abs(ts->input_last_used, ABS_MT_POSITION_X,
					ts->prv_mt_pos[id][CY_XPOS]);
		input_report_abs(ts->input_last_used, ABS_MT_POSITION_Y,
					ts->prv_mt_pos[id][CY_YPOS]);

		if (mt_sync_func)
			mt_sync_func(ts->input_last_used);
		ts->act_trk[id] = CY_NTCH;
		ts->prv_mt_pos[id][CY_XPOS] = 0;
		ts->prv_mt_pos[id][CY_YPOS] = 0;
	}

	if (input_device != ts->input_last_used)
		input_sync(ts->input_last_used);

	/* set Multi-Touch current event signals */
	for (index = 0; index < CY_NUM_MT_TCH_ID; index++) {
		id = t->cur_mt_tch[index];
		if (id >= CY_NUM_TRK_ID)
			continue;

		input_report_abs(input_device, ABS_MT_TRACKING_ID,
						t->cur_mt_tch[index] - 1);
		input_report_abs(input_device, ABS_MT_TOUCH_MAJOR,
						t->cur_mt_z[index]);
		input_report_abs(input_device, ABS_MT_WIDTH_MAJOR,
						t->tool_width);
		input_report_abs(input_device, ABS_MT_POSITION_X,
						t->cur_mt_pos[index][CY_XPOS]);
		input_report_abs(input_device, ABS_MT_POSITION_Y,
						t->cur_mt_pos[index][CY_YPOS]);
		if (mt_sync_func)
			mt_sync_func(input_device);
	...
	ts->input_last_used = input_device;
	return;
no_track_id:
...
static irqreturn_t cyttsp_irq(int irq, void *handle)
{
	...
	input_device = (ts->platform_data->use_airtouch && ts->is_hovering) ?
				ts->input_airtouch : ts->input;

	if (ts->platform_data->use_st)
		handle_single_touch(&xy_data, &trc, ts);

	if (ts->platform_data->use_mt)
		handle_multi_touch(&trc, ts, input_device);

	/* handle gestures */
	if (ts->platform_data->use_gestures && xy_data.gest_id) {
		input_report_key(input_device, BTN_3, CY_TCH);
		input_report_abs(input_device, ABS_HAT1X, xy_data.gest_id);
		input_report_abs(input_device, ABS_HAT2Y, xy_data.gest_cnt);
	}
...
static void configure_airtouch(struct cyttsp *ts)
{
	...
	if (!ts->platform_data->use_airtouch || ts->airtouch_enabled ==
					ts->airtouch_enabled_requested) {
		/* Airtouch is not used, or already configued with the correct
		   setting. */
		return;
	}
	...
	mode = ts->airtouch_enabled_requested ?
				CY_SCAN_MODE_HOVER : CY_SCAN_MODE_NO_HOVER;
	...
	if (!retval)
		ts->airtouch_enabled = ts->airtouch_enabled_requested;
	else
...
static void cyttsp_ts_late_resume(struct early_suspend *h)
{
	...
	if (!ts->fw_loader_mode) {
		...
		/* Airtouch-setting might have changed while the display was
		   suspended. */
...
bypass:
		...
		/* Airtouch setting might have changed while the firmware was
		   updated. */
		msleep(CYTTSP_SCAN_PERIOD);
		configure_airtouch(ts);
	}
...
static int airtouch_dev_open(struct input_dev *dev)
{
	...
	LOCK(ts->mutex);
	ts->airtouch_enabled_requested = 1;
	configure_airtouch(ts);
	UNLOCK(ts->mutex);
	return 0;
}

static void airtouch_dev_close(struct input_dev *dev)
{
	struct cyttsp *ts = dev_get_drvdata(&dev->dev);
	...
	LOCK(ts->mutex);
	ts->airtouch_enabled_requested = 0;
	configure_airtouch(ts);
	UNLOCK(ts->mutex);
}
...
static int register_airtouch(struct cyttsp *ts)
{
	int rc;
	struct input_dev *input_device_airtouch =
			cyttsp_allocate_input_device(ts,
			ts->platform_data->name_airtouch);

	ts->input_airtouch = input_device_airtouch;
	if (input_device_airtouch == NULL)
		return -ENODEV;

	input_device_airtouch->open = airtouch_dev_open;
	input_device_airtouch->close = airtouch_dev_close;
	rc = input_register_device(input_device_airtouch);
	if (rc) {
		dev_err(ts->pdev, "%s: Error, failed to register"
				" input device\n", __func__);
		input_free_device(input_device_airtouch);
		return rc;
	}
	return 0;
}
...
void *cyttsp_core_init(struct cyttsp_bus_ops *bus_ops, struct device *pdev)
{
	...
	if (ts->platform_data->use_airtouch) {
		retval = register_airtouch(ts);
		if (retval)
			goto error_input_register_device_airtouch;
	}

	return ts;

error_input_register_device_airtouch:
	if (ts->platform_data->use_airtouch)
		input_unregister_device(ts->input_airtouch);
error_power_on:
#ifdef CONFIG_TOUCHSCREEN_CYTTSP_CHARGER_MODE
	cancel_work_sync(&ts->charger_status_work);
#endif

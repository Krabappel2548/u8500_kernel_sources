...
static int so34_add_btn_intf(struct so34_data *d, u8 idx, int code, u8 sens)
{
	...
	btn->sens = sens;
	btn->code = code;
	input_set_capability(d->idev, EV_KEY, code);
	return 0;
}
...
{
	int rc = 0;
	u16 buttons = d->idev->users ? d->all_buttons : 0;
	struct device *dev = &d->client->dev;
...
static int so32_idev_open(struct input_dev *idev)
{
	struct so34_data *d = input_get_drvdata(idev);

	dev_dbg(&d->client->dev, "%s\n", __func__);
	so34_req_wake(d, SO34_RESUME_USER);
	return 0;
}

static void so32_idev_close(struct input_dev *idev)
{
	struct so34_data *d = input_get_drvdata(idev);

	dev_dbg(&d->client->dev, "%s\n", __func__);
	so34_req_sleep(d, SO34_SUSPEND_USER);
}
			...
			const struct i2c_device_id *id)
{
	...
	d->idev->open = so32_idev_open;
	d->idev->close = so32_idev_close;

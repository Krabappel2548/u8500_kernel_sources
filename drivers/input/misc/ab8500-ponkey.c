...
struct ab8500_ponkey_info {
	...
	int			irq_dbf;
	int			irq_dbr;
	struct wake_lock	wakeup_wakelock;
};
...
#endif
		wake_lock_timeout(&info->wakeup_wakelock, HZ / 2);
...
#endif
	wake_lock_init(&info->wakeup_wakelock,
			WAKE_LOCK_SUSPEND, "ponkeywake_lock");
...
static int __devexit ab8500_ponkey_remove(struct platform_device *pdev)
{
	...
	wake_lock_destroy(&info->wakeup_wakelock);
#ifdef CONFIG_INPUT_AB8500_FORCECRASH

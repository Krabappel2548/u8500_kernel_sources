...
static void sysrq_handle_forced_crash(int key, struct tty_struct *tty)
{
	char *killer = NULL;

	printk(KERN_EMERG "Force crash triggered!!!\n");
	reboot_reason_set_preset("forced-crash"); /* Inform why we're crashing */
	panic_on_oops = '1';	              /* force handler to use panic() */
...
EXPORT_SYMBOL(ab8500_forced_key_detect);
...
EXPORT_SYMBOL(ab8500_forcecrash_init);
...
EXPORT_SYMBOL(ab8500_forcecrash_exit);

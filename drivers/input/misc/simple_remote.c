						  ...
						  const char *buf, size_t count)
{
	...
	if (!strcmp(SIMPLE_REMOTE_ACC_MIN_VALS_NAME, attr->attr.name)) {
		...
		else if (!strcmp(SIMPLE_REMOTE_TRIG_PRD_F_NAME,
							attr->attr.name))
			ret = jack->interface->set_period_freq(val);
		else if (!strcmp(SIMPLE_REMOTE_TRIG_PRD_T_NAME,
							attr->attr.name))
			ret = jack->interface->set_period_time(val);
		else if (!strcmp(SIMPLE_REMOTE_TRIG_HST_F_NAME,
							attr->attr.name))
			ret = jack->interface->set_hysteresis_freq(val);
		else if (!strcmp(SIMPLE_REMOTE_TRIG_HST_T_NAME,
							attr->attr.name))

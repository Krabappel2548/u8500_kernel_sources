...
/**
 * i2s_stop() - Main i2s stop function.
 * @i2s_cont: i2s controller structure passed by client driver.
 * @message: i2s message structure contains info.
 * Context: process or interrupt.
 *
 * This API is called by client i2s driver as i2s_stop funtion. It will handle
 * i2s stop over i2s bus. The controller should registered its own
 * functions using i2s algorithm structure.
 *
 * Returns error(-1) in case of failure or success(0).
 */
int i2s_stop(struct i2s_controller *i2s_cont, struct i2s_message *message)
{
	return i2s_cont->algo->cont_stop(i2s_cont, message);
}
EXPORT_SYMBOL(i2s_stop);

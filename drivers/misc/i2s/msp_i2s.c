...
static int stm_msp_disable(struct msp *msp, int direction,
			   i2s_flag flag);
static int stm_msp_stop(struct msp *msp, i2s_flag flag);
static int stm_msp_stop_wrapper(struct i2s_controller *i2s_cont,
				struct i2s_message *message);
...
static struct i2s_algorithm i2s_algo = {
	...
	.cont_stop = stm_msp_stop_wrapper,
				...
				enum dma_data_direction direction)
{
	...
	msp->infinite = msp->infinite | direction;
				...
				size_t bytes)
{
	...
	msp->infinite = DMA_TO_DEVICE | DMA_FROM_DEVICE ;
	...
	if (msp->work_mode == MSP_DMA_MODE) {
		if (flag == DISABLE_ALL || flag == DISABLE_TRANSMIT) {
			if (msp->tx_pipeid != NULL) {
				...
				dma_release_channel(msp->tx_pipeid);
				msp->tx_pipeid = NULL;
				msp->infinite &= ~DMA_TO_DEVICE;
			}
		}
		if ((flag == DISABLE_ALL || flag == DISABLE_RECEIVE)) {
			if (msp->rx_pipeid != NULL) {
				if (msp->infinite & DMA_FROM_DEVICE) {
					stedma40_cyclic_stop(msp->rx_pipeid);
					stedma40_cyclic_free(msp->rx_pipeid);
				}
				...
				dma_release_channel(msp->rx_pipeid);
				msp->rx_pipeid = NULL;
				msp->infinite &= ~DMA_FROM_DEVICE;
			}
		}

		if ((msp->rx_pipeid == NULL) && (msp->tx_pipeid == NULL))
			msp->infinite = 0;
	}
...
/**
 * stm_msp_stop() - only disables the given msp controller
 * @msp: specifies the msp contoller data
 * @flag: It indicates the functionality that needs to be disabled.
 *
 * Returns error(-1) in case of failure else success(0)
...
static int stm_msp_stop(struct msp *msp, i2s_flag flag)
{
	...
	if (msp->work_mode == MSP_DMA_MODE) {
		/* check to find if it is DMA/POLLING/INTERRUPT */
		if (flag == DISABLE_ALL || flag == DISABLE_TRANSMIT) {
			/* if TX disable or disable all ,then stop(tx) */
			if (msp->tx_pipeid != NULL) {
		...
		if ((flag == DISABLE_ALL || flag == DISABLE_RECEIVE)) {
			/* if RX disable or disable all ,then stop(rx) */
			if (msp->rx_pipeid != NULL) {
				/* ensure RX is not null */
				if (msp->infinite && DMA_FROM_DEVICE) {
					stedma40_cyclic_stop(msp->rx_pipeid);
					stedma40_cyclic_free(msp->rx_pipeid);
					msp->infinite &= ~DMA_FROM_DEVICE;
...
/**
 * stm_msp_stop_wrapper() - only acts as a wrapper to stm_msp_stop
						...
						struct i2s_message *message)
{
	int status = 0;
	struct msp *msp = (struct msp *)i2s_cont->data;
	int flag = message->i2s_direction;

	/* We need to call it twice for DISABLE_ALL*/
	switch (message->i2s_transfer_mode) {
	case I2S_TRANSFER_MODE_CYCLIC_DMA:
		status = stm_msp_stop(msp, flag);
		break;
	case I2S_TRANSFER_MODE_INF_LOOPBACK:
	case I2S_TRANSFER_MODE_SINGLE_DMA:
	default:

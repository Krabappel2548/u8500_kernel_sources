...
#define MAX_READ_VOLT_SAMPLE 11
 ...
 * @need_bat_init	Detect or not a battery type
	...
	bool enable)
{
	...
	if (di->bat->adc_therm == ADC_THERM_BATCTRL && enable) {
		...
		/*
		 * We need to wait some time, due to switching current source
		 * from 7 to 20 uA does not always work without any delay.
		 */
...
static void ab8500_btemp_periodic_work(struct work_struct *work)
{
	...
	/*
	 * It's really better to identify the battery
	 * here in the separate thread, due to it takes
	 * much time. Therefore, it's a bad idea do it
	 * in "probe" function, when everyone gets stuck
	 * till end of measurement.
	 */
	if (di->need_bat_init) {
		rv = ab8500_btemp_id(di);
		if (rv < 0)
			dev_warn(di->dev, "failed to identify the battery\n");
		else
			di->need_bat_init = false;
	}
...
static irqreturn_t ab8500_btemp_templow_handler(int irq, void *_di)
{
	...
	case AB8500_CUT3P3:
		dev_dbg(di->dev, "Ignore false btemp low irq"
			" for ABB cut 1.0, 1.1, 2.0, 3.0 and 3.3\n");
...
static int __devinit ab8500_btemp_probe(struct platform_device *pdev)
{
	...
	/*
	 * tell, we need to identify the battery
	 */
	di->need_bat_init = true;

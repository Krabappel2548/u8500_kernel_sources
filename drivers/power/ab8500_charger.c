 ...
 * @real_max_usb_in_curr:
 *			The real maximum USB charger input current
	...
	bool connected)
{
	if (connected != di->usb.charger_connected) {
		...
		if (!connected) {
			memset(di->real_max_usb_in_curr, 0,
			       sizeof(di->real_max_usb_in_curr));
		...
		if (connected) {
			...
			if (!wake_lock_active(
				    &di->usb_charger_attached_wake_lock))
	...
	enum ab8500_charger_link_status link_status)
{
	...
	di->real_max_usb_in_curr[0] = di->max_usb_in_curr;
		...
		int ich_in)
{
	...
	if (di->real_max_usb_in_curr[0] > 0)
		min_value = min(di->real_max_usb_in_curr[0], min_value);
...
static void ab8500_charger_vbus_drop_end_work(struct work_struct *work)
{
	...
	if (ret < 0) {
		...
		int auto_curr_index = reg_value >> AUTO_VBUS_IN_CURR_LIM_SHIFT;
		int curr = ab8500_charger_vbus_in_curr_map[auto_curr_index];
		if (di->real_max_usb_in_curr[1] != curr) {
			/* USB source is collapsing */
			di->real_max_usb_in_curr[1] = curr;
			dev_info(di->dev,
				 "VBUS input current limiting to %d mA\n",
				 di->real_max_usb_in_curr[1]);
		} else {
			/* USB source can not give more than this amount.
			 * Taking more will collapse the source.
			 */
			di->real_max_usb_in_curr[0] =
				di->real_max_usb_in_curr[1];
			dev_info(di->dev,
...
/*
 * Force VBUS input current to customized value in mA.
 *
 * NOTE: Do not use this function unless you know what you are doing.
 * Setting current to unsupported level can damage USB data communication.
 */
static ssize_t charger_write_write(struct file *file,
		const char __user *ubuf, size_t count, loff_t *ppos)
{

	struct seq_file *s = file->private_data;
	struct ab8500_charger *di = s->private;
	int rc = count;
	s32 ma;

	if (!read_sysfs_interface(ubuf, &ma, 10) &&
	    ma >= 0 && ma <= USHRT_MAX) {
		/* USBChInputCurr: current that can be drawn from the usb */
		int ret = ab8500_charger_set_vbus_in_curr(di, ma);
		if (ret < 0) {
			dev_err(di->dev, "set VBUS current failed\n");
			rc = ret;
		}
	} else {
		dev_err(di->dev, "Wrong input to sysfs. "
			"Expect [0..%u] mA\n", USHRT_MAX);
		rc = -EINVAL;
	}
...
static const struct file_operations write_fops = {
	.owner			= THIS_MODULE,
	.open			= write_open,
	.write			= charger_write_write,
	.llseek			= seq_lseek,
	.release		= single_release,
};

static struct dentry *chargalg_create_debugfs(struct ab8500_charger *di)
{
	return debugfs_create_file("set_vbus_curr", 0222, 0, di, &write_fops);
}
...
#ifdef CONFIG_DEBUG_FS
	di->debug_file = chargalg_create_debugfs(di);
	if (!di->debug_file)
		dev_err(di->dev, "failed to create debugfs\n");
#endif
	return ret;

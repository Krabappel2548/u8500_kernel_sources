...
static ssize_t chargalg_write_write(struct file *file,
		const char __user *ubuf, size_t count, loff_t *ppos)
{
	struct seq_file *s = file->private_data;
	struct ab8500_chargalg *di = s->private;
	int rc = count;
	s32 ma;

	if (!read_sysfs_interface(ubuf, &ma, 10) &&
	    ma >= 0 && ma <= USHRT_MAX) {
		int ret = ab8500_chargalg_update_chg_curr(di, ma);
		if (ret < 0) {
			dev_err(di->dev, "set charger output current failed\n");
			rc = ret;
		}
	} else {
...
static const struct file_operations write_fops = {
	.owner = THIS_MODULE,
	.open = write_open,
	.write = chargalg_write_write,
	.llseek = seq_lseek,
	.release = single_release,
};

static struct dentry *chargalg_create_debugfs(struct ab8500_chargalg *di)
{
	return debugfs_create_file("set_chg_curr", 0222, 0, di, &write_fops);
}

...
static int av8100_reg_b_r(u8 *analogue_cci);
...
static void enter_scan_mode(struct av8100_device *adev)
{
...
static void av8100_1s_timer_event_handle(struct av8100_device *adev)
{
	...
	/*
	 * This sequence is needed in order to detect short circuit condition
	 * when 5V is always on, since CCI interrupt is not generated in scan
	 * mode when 5V is always on.
	 */
	...
	/* Wait for ONI */
	while (oni == 0 && cnt < ONI_READ_MAX) {
		if (av8100_reg_stby_pend_int_r(NULL, NULL, &oni, NULL, NULL)) {
			err_msg = 3;
			goto av8100_1s_timer_event_handle_err;
		}
		if (oni)
	...
	/* Read CCIflag from internal reg */
	i2c = adev->config.client;
	buf[0] = (AV8100_INTERNAL_REG_1 & 0xff00) >> 8;
	buf[1] = AV8100_INTERNAL_REG_1 & 0xff;

	/* Set address for internal register */
	LOCK_AV8100_HW;
	if (write_multi_byte(i2c, AV8100_REGISTER_D, buf, 2)) {
		UNLOCK_AV8100_HW;
		err_msg = 5;
		goto av8100_1s_timer_event_handle_err;
	}
	...
	if (av8100_reg_gen_ctrl_w(
		...
		AV8100_GENERAL_CONTROL_RA_HIGH)) {
		err_msg = 6;
		goto av8100_1s_timer_event_handle_err;
	}

	/* Read cci in internal register */
	if (av8100_reg_b_r(&cci)) {
		err_msg = 7;
		goto av8100_1s_timer_event_handle_err;
	}
	if (cci) {
...
static void av8100_powerdown_timer_event_handle(struct av8100_device *adev)
{
	...
	if (adev->wakelock_taken) {
		adev->wakelock_taken = false;
		wake_unlock(&adev->wakelock);
	}
}
...
static int av8100_reg_b_r(u8 *analogue_cci)
{
	...
	/* Read from register */
	retval = register_read_internal(AV8100_REGISTER_B, &val);
	/* Set return params */
	if (analogue_cci)
		*analogue_cci = AV8100_REGISTER_B_ANCCI_GET(val);
...
int av8100_hdmi_put(void)
{
	...
	if (adev->params.suspended && !adev->wakelock_taken) {
...
static int av8100_5V_w(u8 denc_off, u8 hdmi_off, u8 on)
{
	...
	if (av8100_status_get().av8100_state > AV8100_OPMODE_STANDBY)
		dev_dbg(adev->dev, "pulsing_5V %d\n", adev->params.pulsing_5V);
	...
	const struct i2c_device_id *id)
{
	...
	wake_lock_init(&adev->wakelock, WAKE_LOCK_SUSPEND, "av8100_wakelock");
err2:
	(void) av8100_powerdown();
err1:
	return ret;
...
static int __devexit av8100_remove(struct i2c_client *i2c_client)
{
	...
	wake_lock_destroy(&adev->wakelock);

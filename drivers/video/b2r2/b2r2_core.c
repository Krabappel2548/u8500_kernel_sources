...
static void domain_disable_work_function(struct work_struct *work)
{
	...
	if (b2r2_core.domain_request_count == 0) {
		...
		if (b2r2_core.vana_reg)
			regulator_disable(b2r2_core.vana_reg);
...
static int domain_enable(void)
{
	...
	if (!b2r2_core.domain_enabled) {
		...
		/* VANA is tighly coupled to DSS EPOD */
		if (b2r2_core.vana_reg)
			WARN_ON_ONCE(regulator_enable(b2r2_core.vana_reg));
...
init_hw_failed:
	...
	if (b2r2_core.vana_reg)
		WARN_ON_ONCE(regulator_disable(b2r2_core.vana_reg));
...
static int init_hw(void)
{
	...
	if (b2r2_core.debugfs_root_dir && !b2r2_core.debugfs_regs_dir) {
		...
		if (!IS_ERR_OR_NULL(b2r2_core.debugfs_regs_dir)) {
			...
			b2r2_log_warn("%s: Failed to create debugfs folder (root=0x%08X, result=0x%08X)\n",
...
static int b2r2_probe(struct platform_device *pdev)
{
	...
	b2r2_core.vana_reg = regulator_get(&pdev->dev, "v-ana");
	/* For some platforms vana does not exist as a regulator */
	if (IS_ERR(b2r2_core.vana_reg))
		b2r2_log_err("regulator_get vana failed (dev_name=%s)\n",
				dev_name(&pdev->dev));
...
b2r2_probe_no_res:
	if (b2r2_core.vana_reg)
		regulator_put(b2r2_core.vana_reg);

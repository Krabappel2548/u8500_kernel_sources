...
#define PANEL_RESET_DELAY_1 11 /* Worst spec says > 10 ms */
#define PANEL_RESET_DELAY_2 1  /* Worst spec says > 20 us */
#define PANEL_RESET_DELAY_3 33  /* Worst spec says > 32 ms from reset high */
...
static int platform_startup_reset(struct mcde_display_device *ddev)
{
	...
	if (ddev->platform_reset) {
		...
		if (ret)
			goto exit;
		msleep(PANEL_RESET_DELAY_1);
		ret = ddev->platform_reset(ddev, 0);
		if (ret)
			goto exit;
		mdelay(PANEL_RESET_DELAY_2);
		ret = ddev->platform_reset(ddev, 1);
		if (ret)
			goto exit;
		msleep(PANEL_RESET_DELAY_3 - PANEL_RESET_DELAY_2 -
							PANEL_RESET_DELAY_1);
	}
exit:
	return ret;
...
static int __devinit panel_probe(struct mcde_display_device *ddev)
{
	...
	if (rd->panel->disable_mcde_when_releasing_ovly1)
		mcde_disable_mcde_when_releasing_ovly1_support(true);

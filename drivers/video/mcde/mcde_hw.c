...
static bool disable_mcde_when_releasing_ovly1;
...
static void dsi_link_handle_reset(u8 link, bool release)
{
	...
	reg = prcmu_read(DB8500_PRCM_DSI_SW_RESET);
	if (release) {
		switch (link) {
		case 0:
			value = reg |
				DB8500_PRCM_DSI_SW_RESET_DSI0_SW_RESETN;
			break;
		case 1:
			value = reg |
				DB8500_PRCM_DSI_SW_RESET_DSI1_SW_RESETN;
			break;
		case 2:
			value = reg |
				DB8500_PRCM_DSI_SW_RESET_DSI2_SW_RESETN;
			break;
		default:
			value = reg;
			break;
...
static void dsi_link_switch_byte_clk(u8 link, bool to_system_clock)
{
	...
	reg = prcmu_read(DB8500_PRCM_DSI_GLITCHFREE_EN);
	if (to_system_clock) {
		switch (link) {
		case 0:
			value = reg |
				DB8500_PRCM_DSI_GLITCHFREE_EN_DSI0_BYTE_CLK;
			break;
		case 1:
			value = reg |
				DB8500_PRCM_DSI_GLITCHFREE_EN_DSI1_BYTE_CLK;
			break;
		case 2:
			value = reg |
				DB8500_PRCM_DSI_GLITCHFREE_EN_DSI2_BYTE_CLK;
			break;
		default:
			value = reg;
			break;
...
static void dsi_link_enable(struct mcde_chnl_state *chnl)
{
...
static irqreturn_t mcde_irq_handler(int irq, void *dev)
{
	...
	if (irq_status & MCDE_RISPP_VSCC0RIS_MASK) {
		mcde_handle_vsync(&channels[MCDE_CHNL_A]);
		mcde_handle_vsync(&channels[MCDE_CHNL_B]);
		mcde_handle_vsync(&channels[MCDE_CHNL_C0]);
		mcde_handle_vsync(&channels[MCDE_CHNL_C1]);
	}
						...
						struct mcde_ovly_state *ovly)
{
	...
	if (regs->enabled && ovly->pix_fmt == MCDE_OVLYPIXFMT_YCbCr422) {
		...
		chnl->regs.background_is_yuv = true;
		...
		if (!chnl->oled_color_conversion) {
			...
			ovly0->regs.dirty = true;
			ovly1->regs.dirty = true;
			chnl->regs.background_is_yuv = false;
						...
						struct mcde_ovly_state *ovly)
{
	...
	if (port->type == MCDE_PORTTYPE_DPI && port->phy.dpi.tv_mode) {
		...
		if (port->pixel_format != MCDE_PORTPIXFMT_DSI_YCBCR422) {
			/* Only activate oled registers for CHNL_A (lcd) */
			if (chnl->id == MCDE_CHNL_B) {
				if (ovly->pix_fmt != MCDE_OVLYPIXFMT_YCbCr422) {
				...
				setup_channel_and_overlay_color_conv(chnl,
									ovly);
		} else {
			if (ovly->pix_fmt != MCDE_OVLYPIXFMT_YCbCr422)
				/* DSI: RGB -> YUV */
			...
			enum mcde_display_rotation rotation)
{
	...
	default:
		...
		if (regs->enabled) {
			dev_warn(&mcde_dev->dev,
			"Default pixelfetchwtrmrklevel used for ovly%d\n", idx);
			WARN_ON_ONCE(true);
		}
		break;
	}
...
static void stop_channel(struct mcde_chnl_state *chnl)
{
	 ...
	 * disabled for
	 * 1) video mode
	 * 2) TE0 vsync in DSI command mode
	*/
	if (chnl->port.update_auto_trig ||
				(chnl->port.sync_src == MCDE_SYNCSRC_TE0 &&
					chnl->port.type == MCDE_PORTTYPE_DSI))
	...
	if (chnl->port.type == MCDE_PORTTYPE_DSI) {
		if (chnl->regs.synchronized_update) {
			if (chnl->port.sync_src == MCDE_SYNCSRC_BTA) {
				...
				mcde_wfld(MCDE_CRC, SYCEN0, true);
				enable_flow(chnl);
			} else if (chnl->port.sync_src == MCDE_SYNCSRC_OFF) {
				do_softwaretrig(chnl);
			}
		} else {
			do_softwaretrig(chnl);
		...
		dev_err(&mcde_dev->dev, "%s: No refresh, chnl=%d badly "
					"configured\n", __func__, chnl->id);
	}
}
...
void mcde_disable_mcde_when_releasing_ovly1_support(bool disable)
{
	mcde_lock(__func__, __LINE__);
	if (disable)
		disable_mcde_when_releasing_ovly1 = true;
	else
		disable_mcde_when_releasing_ovly1 = false;
	mcde_unlock(__func__, __LINE__);
}
...
void mcde_ovly_put(struct mcde_ovly_state *ovly)
{
	...
	/* Only works when using one extra overlay. Do not use this when hdmi */
	/* is enabled */
	/* TODO: Remove when problem is solved */
	if (disable_mcde_when_releasing_ovly1 && ovly->idx == 1) {
		mcde_lock(__func__, __LINE__);
...
static int init_clocks_and_power(struct platform_device *pdev)
{
	...
	/*
	 * DPI clocks are not necessary to get
	 * There can be platforms that do not have DPI support
	 */
